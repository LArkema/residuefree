...
if [ -z "$($SUDO podman images|grep $IMGNAME)" ]; then
...
CMD ["/sbin/init"]
EOF
    $SUDO podman build -t $IMGNAME --force-rm=true --rm=true $BUILDDIR || clean_up
fi
...
while [ "$($SUDO podman exec $CONTNAME sh -c 'systemctl status snapd.seeded >/dev/null 2>&1; echo $?')" != "0" ]; do
    echo -n "."
...
$SUDO podman exec $CONTNAME snap install core --edge || clean_up
